...
contract Farm is VRFConsumerBaseV2Plus {
    ...
    function _initializePlayer(address _player) internal {
        // 初始信息
    ...
    function _req_plotId(uint256 _plotId) internal view {
        // 统一验证
        require(_plotId > 0, "Plot ID must be greater than zero");
        require(
            _plotId <= players[msg.sender].unlockedPlots,
            "Plot not unlocked"
        );
    }

    function read_plot(uint256 _plotId) public view returns (Plot memory) {
        // 返回当前地址每块地的信息
        _req_plotId(_plotId);
    ...
    function sow(uint256 _plotId) public {
        // 播种 + 随机奖励升级
        _req_plotId(_plotId);
    ...
    function harvest(uint256 _plotId) public {
        // 收获
        _req_plotId(_plotId);
        Plot storage plot = players[msg.sender].plots[_plotId];
        require(
            plot.harvestTime <= block.timestamp,
    ...
    function water(uint256 _plotId) public {
        // 浇水
        _req_plotId(_plotId);
        Plot storage _plot = players[msg.sender].plots[_plotId];
        require(
            _plot.status == 1,
            "Plot is not in the correct state for watering"
        );
        require(
            _plot.harvestTime > block.timestamp,
            "No need to plant, you can harvest"
        );
        require(
            _plot.lastWatered + 4 hours <= block.timestamp,
            "Water no more than Four times a day"
        );
    ...
    function buy_seed(uint256 _seedAmount) public {
        // 买种子
        Player storage player = players[msg.sender];
    ...
    function sell_outcome(uint256 _outcomeAmount) public {
        // 卖出成果
        ...
        Player storage player = players[msg.sender];
        require(
            _outcomeAmount <= player.outcomeAmount && _outcomeAmount > 0,
    ...
    function updatePriceData() internal {
        // 获取链上价格更新每天卖出用
